...
gulp.task('deploy:cloudformation', function(callback) {
    var cloudformation_template = 'cloudformation/' + gutil.env.env + '/flex-cloudformation-brightcove-callback.json';
...
gulp.task('deploy:lambda-zip', function() {
    gulp.src('./index.js')
        .pipe(zip('brightcove-callback-api.zip'))
        .pipe(gulp.dest('.'));
});

gulp.task('deploy:lambda-upload', function() {
    gulp.src('brightcove-callback-api.zip')
        .pipe(s3({
